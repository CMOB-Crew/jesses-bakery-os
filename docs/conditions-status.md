...
| 7 | Rate limiting on login and reset | **partly** | **Changed 14 Sept.** Five failures a minute per identifier on both sign-in and reset, with every attempt logged to `login_attempts` — the condition asks for both and a limiter that counts in memory would satisfy neither on Netlify, where each function instance has its own memory. Asserted in `scripts/login-rate-limit-check.ts` and the three `security definer` functions were exercised against a real Postgres. **Partly, not met, until migration 100 is applied to production** — the code is live-safe without it (the limiter fails open and says so in the function log) which is exactly why it must not be assumed done. |
...
| 9 | PITR plus an independent weekly dump | **not met** | **Changed 14 Sept, still not met.** Point-in-time recovery is an unpurchased add-on and was declined on 11 Sept for stated reasons; the project has seven daily backups and nothing finer. Supabase's own backups page states storage objects are not included — so **the delivery photographs are still in no backup of any kind.** The sync now exists and is asserted end to end against fakes (`scripts/backup-sync-check.ts`), with `backup_runs` / `backup_objects` in migration 099 and a staleness alarm, because a backup that has stopped and one with nothing to do look identical from outside. **It stays not met until an object is actually copied**, which needs Jesse's consent on `Sites.Selected` and the Backups library. Route, schedule and restore test are not built. |
...
**Ten met. One not met (9). Four partly met (2, 4, 7, 8).**
